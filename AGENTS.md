...
* A focused tree row may still be offscreen or inside a collapsed ancestor: scroll a rendered focused row only within its tree scroller, and use `expandDocTree` for hidden rows. Reconcile the active document after layout readiness and feature re-enablement; verify visible, offscreen, collapsed, and database-page cases in an isolated desktop session.
* Document-tree location must skip pages that contain a database and database-backed row pages when the database-page guard is enabled; apply this to both automatic and plugin button-triggered location without intercepting SiYuan's native tree controls.
...
* Resolve the database field-panel toggle across SiYuan DOM versions: newer releases use `.protyle-db-attr__header > .protyle-db-attr__toggle[data-type="toggle"]`, while older releases place `data-type="toggle"` on the header itself.
